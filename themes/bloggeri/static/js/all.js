...
if (typeof(Storage) !== "undefined") {
  ...
  document.getElementById("createFolderBtn").addEventListener("click", function() {
    ...
    if (folderName) {
      // Get existing folders from local storage or initialize an empty array
      var folders = JSON.parse(localStorage.getItem("folders")) || [];
      
      // Add the new folder to the array
      folders.push(folderName);
      
      // Save the updated folder array back to local storage
...
function displayFolders(folders) {
  ...
  // Create list items for each folder
...
function renameFolder(folder) {
  var newName = prompt("Enter a new name for the folder:", folder);
  
  if (newName && newName !== folder) {
    var folders = JSON.parse(localStorage.getItem("folders")) || [];
    
    // Find the index of the folder in the array
    var index = folders.indexOf(folder);
    
    if (index > -1) {
      // Update the folder name in the array
